...
class FPPLC3(PLC):
    ...
    def main_loop(self, sleep=0.0):
        ...
        logging.basicConfig(filename='logs/plc3.log',
                            format='%(levelname)s %(asctime)s '+PLC3_ADDR+' '+PLC3_ADDR+' %(message)s', datefmt='%m/%d/%Y %H:%M:%S', level=logging.DEBUG)
        ...
        while True:
            ...
            try:
                ...
                logging.info("value-update-successful plc3:: INFO: successfully updated value from sensor 3: %.2f" % (liquidlevel_bottle))

            except:
                logging.warning("value-update-FAILED plc3:: WARNING: failed to update value from sensor 3")
