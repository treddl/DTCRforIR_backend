...
class FPPLC2(PLC):
    ...
    def main_loop(self, sleep=0.0):
        ...
        logging.basicConfig(filename='logs/plc2.log',
                            format='%(levelname)s %(asctime)s '+PLC2_ADDR+' '+PLC2_ADDR+' %(message)s', datefmt='%m/%d/%Y %H:%M:%S', level=logging.DEBUG)
        ...
        while True:
           ...
           # print "PLC2 - get flowlevel (SENSOR 2): %f" % flowlevel
            ...
            try:
                ...
                logging.info("value-update-successful plc2:: INFO: successfully updated value from sensor 2: %.2f" % (flowlevel))
            except:
                logging.warning("value-update-FAILED plc2:: WARNING: failed to update value from sensor 2")
